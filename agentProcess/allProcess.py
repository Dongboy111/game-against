...
def clear():
    for i in range(agent1Num):
        ...
        agent1PathPoints[i].clear()
        agent2PathPoints[i].clear()
    agent1AllAgentNowPoints.clear()
    agent2AllAgentNowPoints.clear()

    agent1PublicPoints.clear()
    agent2PublicPoints.clear()
...
class allProcess:
    ...
    def move(self,action,start_time,newDone):#两组agent共用的
        ...
        if newDone:
            ...
            for i in range(12, 15):
                for j in range(18, 21):
                    agent2PublicPoints.append((j, i))
            # 初始化智能体1的初始点位
            for i in range(self.agentNum[0]):
                agent1AllAgentNowPoints.append(random.choice(agent1PublicPoints))
            ...
            # eg.msgbox("qingkongle","qingkongle")
            # print(agent1PublicPoints)
            # import time
            # time.sleep(10)
        ...
        for i in range(len(action[0])):
            ...
            for  item in range(len(agent2PathPoints)):
                if sum in agent2PathPoints[item] :
                    ...
                    agentReward[0] = agentReward[0] + 20
                    agentReward[1] = agentReward[1] - 5
            ...
            if sum in agent1PublicPoints:
                ...
                # global agent2PublicPoints
                # agent2PublicPoints = [i for i in agent2PathPoints if i not in pub1TempPoint]
                agent1PathPoints[i] = []
                agentReward[0] = agentReward[0] + 20
        ...
        for i in range(len(action[1])):
            ...
            for item in range(len(agent2PathPoints)):
                if sum in agent1PathPoints[item]:
                    ...
                    agentReward[0] = agentReward[0] - 5
                    agentReward[1] = agentReward[1] + 20
            # 是否到达公共区域检测  如果到达 则进行闭操作
            if sum in agent2PublicPoints:
                ...
                if diyige==[] or dierge==[]:
                    ...
                    # kkk= [i for i in agent1PathPoints if i not in pub1TempPoint]
                    # global agent1PublicPoints
                    # agent1PublicPoints = kkk
                    agent2PathPoints[i] = []
        ...
        agentReward[1] = agentReward[1] + extenR2
        if agentReward[0]>80:
            agentReward[0]=80
        if agentReward[0]<-80:
            agentReward[0]=-80
    ...
    def drawAlllByFive(self):
        ...
        for i in agent1PublicPoints:
            # print(i)
            # print(np.array(self.allPoints).shape)
            # print(self.allPoints)
            #
            # print("zheshsiyigexioaglakjkdjflajklaenklja;lfdssdg")
...
def getRewardByDis(team):
    ...
    midDisLen = 50#是奖励值递减的分界线 具体见 计算公式里面的
    if team==0:
        len1 = []
        dis1 = []
